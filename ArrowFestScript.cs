...
public class ArrowFestScript : MonoBehaviour
{
    ...
    // Arrow Fest Oyun Mekaniði
    // Hata Veya Soru Ýçin Discord DRAÇ#7981 mesaj gönderebilirsiniz
    ...
    // Oyun Çalýþmýyorken editor penceresinde bir deðiþiklik yapýldýðýnda örneðin (Arrow sayýsýný arttýrdýðýmýzda) aþaðýdaki OnValidate fonksiyonu çalýþýr
    // Yani oyunu çalýþtýrmadan Editorden ayar yapmaya yara
    // Ama bunun için Package Manager'den Editor Coroutines paketini idnirmelisiniz
    ...
    private void DestroyArrows()
    {
        for (int i = arrows.Count - 1; i >= ArrowCount; i--)
        {
            ...
            arrows.RemoveAt(i);
            EditorCoroutineUtility.StartCoroutine(DestroyObject(arrowClone), this); //Destroy iþlemini editörde yapmak için
        }
